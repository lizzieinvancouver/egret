# Started Feb 10, 2025 - last updated March 2026
...
d$nseeds_total <- as.numeric(d$n.per.rep)*as.numeric(d$reps)
...
# RESPONSE
resp <- c("responseVar", "responseValueNum", "nseeds_total")

# Unique treatments per study
dtreat <- unique(d[,treats])
dtreat$uniqueID <- 1:nrow(dtreat)
d <- unique(d[, c(treats, resp, 'figure')])
d <- merge(x = d, y = dtreat, by = treats, all = TRUE)
d$genusspecies <- paste0(d$genus, '_', d$species)


originald[originald$datasetID %in% 'nurse08' & originald$responseVar %in% 'percent.germ', c('Notes','responseVar')]
test <- originald[originald$datasetID %in% 'nurse08' & originald$responseVar %in% 'percent.germ' & originald$study %in% 'exp4', ]
...
# Transform to number of seeds
filteredd <- filteredd[filteredd$nseeds_total != 0 & !is.na(filteredd$nseeds_total),]
filteredd$nseeds <- filteredd$nseeds_total*(filteredd$responseValueNum)/100
...
filteredd$other.treatment.details <- NA

## A particular case of misc. treatment: the color of the seeds
# We want to keep both brown and black, so we're dealing with this (but we still want to save the info somewhere)
# before entering the loop (it's easier, I'm lazy)
filteredd[filteredd$other.treatment %in% c('brown seeds', 'black seeds') &
            filteredd$datasetID == 'nurse08', 'other.treatment.details'] <- 
  filteredd[filteredd$other.treatment %in% c('brown seeds', 'black seeds') &
              filteredd$datasetID == 'nurse08', 'other.treatment']
filteredd[filteredd$other.treatment %in% c('brown seeds', 'black seeds') &
            filteredd$datasetID == 'nurse08', 'other.treatment'] <- 'brown/black seeds'

# Added March 17 - soil kPa was nowhere
filteredd[filteredd$datasetID %in% 'nurse08' & filteredd$figure %in% c('Fig 4a', 'Fig 4e'), 'other.treatment'] <- 
  paste0(filteredd[filteredd$datasetID %in% 'nurse08' & filteredd$figure %in% c('Fig 4a', 'Fig 4e'), 'other.treatment'],
         ' soil at 0kPa')
filteredd[filteredd$datasetID %in% 'nurse08' & filteredd$figure %in% c('Fig 4b', 'Fig 4f'), 'other.treatment'] <- 
  paste0(filteredd[filteredd$datasetID %in% 'nurse08' & filteredd$figure %in% c('Fig 4b', 'Fig 4f'), 'other.treatment'],
         ' soil at -0.32kPa')
filteredd[filteredd$datasetID %in% 'nurse08' & filteredd$figure %in% c('Fig 4c', 'Fig 4g'), 'other.treatment'] <- 
  paste0(filteredd[filteredd$datasetID %in% 'nurse08' & filteredd$figure %in% c('Fig 4c', 'Fig 4g'), 'other.treatment'],
         ' soil at -10kPa')
filteredd[filteredd$datasetID %in% 'nurse08' & filteredd$figure %in% c('Fig 4d', 'Fig 4h'), 'other.treatment'] <- 
  paste0(filteredd[filteredd$datasetID %in% 'nurse08' & filteredd$figure %in% c('Fig 4d', 'Fig 4h'), 'other.treatment'],
         ' soil at -500kPa')
...
filteredd[filteredd$other.treatment %in% c('Brown seed', 'Black seed') &
            filteredd$datasetID == 'li11', 'other.treatment.details'] <- 
  filteredd[filteredd$other.treatment %in% c('Brown seed', 'Black seed') &
              filteredd$datasetID == 'li11', 'other.treatment']
filteredd[filteredd$other.treatment %in% c('Brown seed', 'Black seed') &
...
filteredd[filteredd$datasetID %in% 'alhelal96' & filteredd$study %in% 'exp1', 'other.treatment'] <- filteredd[filteredd$datasetID == 'alhelal96' & filteredd$study == 'exp1', 'treatment'] 

## This should be in other.treatment
filteredd[filteredd$datasetID %in% 'ochuodho08' & filteredd$study %in% 'exp3' & 
            filteredd$treatment %in% c('seed.dark.control', 'seed.dark.strat'), 'other.treatment'] <- 'light/dark seeds'
filteredd[filteredd$datasetID %in%  'ochuodho08' & filteredd$study %in% 'exp3' & 
            filteredd$treatment %in% c('seed.dark.control', 'seed.dark.strat'), 'other.treatment.details'] <- 'dark seeds'
filteredd[filteredd$datasetID %in%  'ochuodho08' &filteredd$study %in% 'exp3' & 
            filteredd$treatment %in% c('seed.light.control', 'seed.light.strat'), 'other.treatment'] <- 'light/dark seeds'
filteredd[filteredd$datasetID %in%  'ochuodho08' & filteredd$study %in% 'exp3' & 
            filteredd$treatment %in% c('seed.light.control', 'seed.light.strat'), 'other.treatment.details'] <- 'light seeds'

# This also should be in other.treatment
filteredd[filteredd$datasetID %in% 'ren15' & filteredd$study %in% 'exp1' & filteredd$treatment %in% 'strong light', 'other.treatment'] <-
  'strong light'
filteredd[filteredd$datasetID %in% 'ren15' & filteredd$study %in% 'exp1' & filteredd$treatment %in% 'weak light', 'other.treatment'] <-
  'weak light'
filteredd[filteredd$datasetID %in% 'ren15' & filteredd$study %in% 'exp1' & filteredd$treatment %in% 'darkness', 'other.treatment'] <-
  'darkness'

# This also should be in other.treatment (I think? Or maybe not, but that's an easy fix)
filteredd[filteredd$datasetID %in% 'kolodziejek18' & filteredd$study %in% 'exp0' & filteredd$treatment %in% 'untreated, light ', 'other.treatment'] <- 
  'light'
filteredd[filteredd$datasetID %in% 'kolodziejek18' & filteredd$study %in% 'exp0' & filteredd$treatment %in% 'untreated, dark', 'other.treatment'] <- 
  'darkness'
filteredd[filteredd$datasetID %in% 'kolodziejek18' & filteredd$study %in% 'exp1' & filteredd$treatment %in% 'dry stored, light ', 'other.treatment'] <- 
  'light'
filteredd[filteredd$datasetID %in% 'kolodziejek18' & filteredd$study %in% 'exp1' & filteredd$treatment %in% 'dry stored, dark', 'other.treatment'] <- 
  'darkness'
filteredd[filteredd$datasetID %in% 'kolodziejek18' & filteredd$study %in% 'exp2' & filteredd$treatment %in% 'cold stratification,light', 'other.treatment'] <- 
  'light'
filteredd[filteredd$datasetID %in% 'kolodziejek18' & filteredd$study %in% 'exp2' & filteredd$treatment %in% 'cold stratification, dark', 'other.treatment'] <- 
  'darkness'

# Added March 17
filteredd[filteredd$datasetID %in% 'jang22' & filteredd$study %in% 'exp7' & filteredd$treatment %in% 'Light', 'other.treatment'] <- 
  'light'
