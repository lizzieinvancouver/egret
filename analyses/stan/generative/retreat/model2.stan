...
parameters {
  ...
  vector[Nspecies] alpha_pv_base; // probability of viability
  ...
  vector[Nspecies] beta_log_delta_tau_forc;
}
...
transformed parameters {
  ...
  for(e in 1:Nexps){
    ...
    real log_delta_tau = mu_log_delta_tau_base + beta_log_delta_tau_forc[species] * (forcing[e]-forcing0);
    // real tau50 = tau1 + exp(log_delta_tau);
    real tau50 = tau1 + exp(log_delta_tau);
...
model {
  ...
  alpha_pv_base ~ normal(mu_pv_base,sigma_pv_base);
  ...
  beta_pv_chill ~ normal(mu_beta_pv_chill, sigma_beta_pv_chill); 
  
  mu_log_tau1_base ~ normal(0,1);
  sigma_log_tau1_base ~ normal(0,0.2); // pif
  log_tau1_base ~ normal(mu_log_tau1_base,sigma_log_tau1_base);
  ...
  mu_beta_log_delta_tau_forc ~ normal(0, 1); // one degree increase would lead to a reduction of 2 days max at delta_tau= 15?
  sigma_beta_log_delta_tau_forc ~ normal(0 , 3);
  beta_log_delta_tau_forc ~ normal(mu_beta_log_delta_tau_forc,sigma_beta_log_delta_tau_forc);
