...
parameters {
  ...
  // slope of time x forcing interaction
  vector[Nsp] btxf; 
  real btxf_z; // root value
  real<lower=0, upper=1> lambda_btxf;  // phylogenetic structure        
  real<lower=0> sigma_btxf; // overall rate of change (brownian motion?)
  
  // slope of chilling x forcing interaction
  vector[Nsp] bcxf; 
  real bcxf_z; // root value
  real<lower=0, upper=1> lambda_bcxf;  // phylogenetic structure        
  real<lower=0> sigma_bcxf; // overall rate of change (brownian motion?)
...
transformed parameters {
  ...
  if(N_degen>0) {
    for(i in 1:N_degen){
      calc_degen[i] = a[sp_degen[i]] + bt[sp_degen[i]] * t_degen[i] + bf[sp_degen[i]] * f_degen[i] + bc[sp_degen[i]] * c_degen[i] +
        btxf[sp_degen[i]] * t_degen[i] * f_degen[i] + bcxf[sp_degen[i]] * c_degen[i] * f_degen[i] ;
    }
  }
  ...
  for(i in 1:N_prop){
    ...
    calc_prop[i] = a[sp_prop[i]] + bt[sp_prop[i]] * t_prop[i] + bf[sp_prop[i]] * f_prop[i] + bc[sp_prop[i]] * c_prop[i] +
      btxf[sp_prop[i]] * t_prop[i] * f_prop[i] + bcxf[sp_prop[i]] * c_prop[i] * f_prop[i] ;
...
model {
  ...
  matrix[Nsp,Nsp]  C_btxf = lambda_btxf * Vphy;
  C_btxf = C_btxf - diag_matrix(diagonal(C_btxf)) + diag_matrix(diagonal(Vphy));
  matrix[Nsp,Nsp]  C_bcxf = lambda_bcxf * Vphy;
  C_bcxf = C_bcxf - diag_matrix(diagonal(C_bcxf)) + diag_matrix(diagonal(Vphy));
  ...
  matrix[Nsp,Nsp] L_bcxf =  cholesky_decompose(sigma_bcxf^2*C_bcxf); 
  matrix[Nsp,Nsp] L_btxf =  cholesky_decompose(sigma_btxf^2*C_btxf); 
  ...
  bcxf ~ multi_normal_cholesky(rep_vector(bcxf_z,Nsp), L_bcxf); 
  btxf ~ multi_normal_cholesky(rep_vector(btxf_z,Nsp), L_btxf); 
  ...
  lambda_bcxf ~ beta(1.5, 1.5);
  sigma_bcxf ~ normal(0, 1);
  
  lambda_btxf ~ beta(1.5, 1.5);
  sigma_btxf ~ normal(0, 1);
