...
functions {
  ...
  // below, for parallelization of log-likelihood sum - for cases where x in (0,1)
  real partial_sum(array[] real y_slice,
                   int start, int end,
                   array[] real calc_prop,
                   vector cutpoints,
                   real kappa) {
    
    real sumloglk = 0;
    int L = end - start + 1;
    array[L] real calc_prop_slice = calc_prop[start:end];
    
    for(i in 1:L) {
      sumloglk += log(inv_logit(calc_prop_slice[i] - cutpoints[1]) - inv_logit(calc_prop_slice[i] - cutpoints[2]));
      sumloglk += beta_proportion_lpdf(y_slice[i] | inv_logit(calc_prop_slice[i]),kappa);
    }
...
data {
  ...
  int grainsize; // parallelization parameter
}
...
model {
  ...
  // for(n in 1:N_prop) {
    ...
    // y_prop[n] ~ beta_proportion(inv_logit(calc_prop[n]),kappa); previous version
    // target += beta_proportion_lpdf(y_prop[n] | inv_logit(calc_prop[n]),kappa);
  // }
  
  // parallel implementation
  target += reduce_sum(partial_sum, y_prop, grainsize, calc_prop, cutpoints, kappa);
