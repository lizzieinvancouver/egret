...
parameters {
  ...
  ordered[2] cutpoints[Nsp]; // cutpoints on ordered (latent) variable (also stand in as intercepts)
...
model {
  ...
  for(s in 1:Nsp){
    ...
    target += induced_dirichlet_lpdf(cutpoints[s, 1:2] | rep_vector(1, 3), 0);
  ...
  if(N_degen>0) {
    for(n in 1:N_degen) {
      if(y_degen[n]==0) {
        // Pr(Y==0)
        target += log1m_inv_logit(calc_degen[n] - cutpoints[sp_degen[n], 1]);
      } else {
        //Pr(Y==1)
        target += log_inv_logit(calc_degen[n] - cutpoints[sp_degen[n], 2]);
      }
    }
  }
  ...
  for(n in 1:N_prop) {
    // Pr(Y in (0,1))
    target += log(inv_logit(calc_prop[n] - cutpoints[sp_prop[n], 1]) - inv_logit(calc_prop[n] - cutpoints[sp_prop[n], 2]));
...
generated quantities {
  ...
  if(N_degen>0) {
    for(i in 1:N_degen) {
      ...
      logisticvariate = ordered_logistic_rng(calc_degen[i], cutpoints[sp_degen[i], 1:2]);
  ...
  for(i in 1:N_prop) {
    ...
    logisticvariate = ordered_logistic_rng(calc_prop[i], cutpoints[sp_prop[i], 1:2]);
