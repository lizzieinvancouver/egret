...
transformed parameters {
  ...
  if(N_degen>0) {
    for(i in 1:N_degen){
      calc_degen[i] = a[sp_degen[i]] //+ bt[sp_degen[i]] * t_degen[i] 
      + bf[sp_degen[i]] * f_degen[i] + bcs[sp_degen[i]] * cs_degen[i];
    }
  }
  
  for(i in 1:N_prop){
    
    calc_prop[i] = a[sp_prop[i]] //+ bt[sp_prop[i]] * t_prop[i] 
    + bf[sp_prop[i]] * f_prop[i] + bcs[sp_prop[i]] * cs_prop[i];
