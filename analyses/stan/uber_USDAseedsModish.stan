// Started OCt30 2025 Dan by dan to modify 
...
model {
       real yhat[N];
        ...
        matrix[n_sp,n_sp] vcv_bc;     // phylogeny
       	...
       	for(i in 1:N){
            yhat[i] = 
	      a[sp[i]] + b_force[sp[i]] * x1[i] + b_chill[sp[i]] * x2[i];
			     	}
 ...
 // Priors -- keep in Stan code, better for reproducibility and runs faster
    a_z ~ normal(62, 29); // Same as before, seems okay
    b_zf ~ normal(0, 10); // updated prior ... I think we should also try 0, 10
    ...
    sigma_interceptsa ~ normal(0, 28);
    sigma_interceptsbf ~ normal(0, 28);
    sigma_interceptsbc ~ normal(0, 28);
    sigma_interceptsbp ~ normal(0, 28);
    
    sigma_y ~ normal(0, 28); // updated prior
