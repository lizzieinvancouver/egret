...
data {
  ...
  vector[N_prop] cs_prop; // covariate coldstrat for proportion outcome
  vector[N_degen] cs_degen; // covariate coldstrat for degenerate (0,1) outcome
  
  vector[N_prop] ws_prop; // covariate warmstrat for proportion outcome
  vector[N_degen] ws_degen; // covariate warmstrat for degenerate (0,1) outcome
...
parameters {
  ...
  // slope of warm strat. effect
  vector[Nsp] bws; 
  real bws_z; // root value
  real<lower=0, upper=1> lambda_bws;  // phylogenetic structure        
  real<lower=0> sigma_bws; // overall rate of change (brownian motion?)
...
transformed parameters {
  ...
  if(N_degen>0) {
    for(i in 1:N_degen){
      calc_degen[i] = a[sp_degen[i]] + bt[sp_degen[i]] * t_degen[i] + bf[sp_degen[i]] * f_degen[i] + bcs[sp_degen[i]] * cs_degen[i] + bws[sp_degen[i]] * ws_degen[i];
    }
  }
  
  for(i in 1:N_prop){
    
    calc_prop[i] = a[sp_prop[i]] + bt[sp_prop[i]] * t_prop[i] + bf[sp_prop[i]] * f_prop[i] + bcs[sp_prop[i]] * cs_prop[i] + bws[sp_prop[i]] * ws_prop[i];
...
model {
  ...
  matrix[Nsp,Nsp]  C_bcs = lambda_bcs * Vphy;
  C_bcs = C_bcs - diag_matrix(diagonal(C_bcs)) + diag_matrix(diagonal(Vphy));
  matrix[Nsp,Nsp]  C_bws = lambda_bws * Vphy;
  C_bws = C_bws - diag_matrix(diagonal(C_bws)) + diag_matrix(diagonal(Vphy));
  ...
  matrix[Nsp,Nsp] L_bcs =  cholesky_decompose(sigma_bcs^2*C_bcs); 
  matrix[Nsp,Nsp] L_bws =  cholesky_decompose(sigma_bws^2*C_bws); 
  ...
  bws ~ multi_normal_cholesky(rep_vector(bws_z,Nsp), L_bws); 
  ...
  lambda_bcs ~ beta(1.5, 1.5);
  sigma_bcs ~ normal(0, 1);
  
  lambda_bws ~ beta(1.5, 1.5);
  sigma_bws ~ normal(0, 1);
