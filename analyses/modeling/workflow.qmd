...
➡ What are the germination responses to different cues? (see <a href="https://github.com/lizzieinvancouver/egret/wiki/Questions">GitHub repo wiki</a>)
...
Let's simulate some data! We assume a priori that species are related through a given phylogenetic tree (so we start from a quite complex model...), with a certain total length $\phi$.  
The following block of code is largely inspired from Deirdre's amazing work for OSPREE.
...
We thus make the hypothesis that the correlation (or covariance) between the species is a function of their relative distance (in the phylogenetic tree),  $\Delta x = | x_{sp_i} - x_{sp_j} |$.  
The maximum distance between two species is thus $\max\Delta x = 2 * \phi$.  
We can then visualize the structure of the covariance function, either with a covariogram or with a correlogram (see <a href="https://betanalpha.github.io/assets/case_studies/gaussian_processes.html">Mike's chapter on Gaussian Processes</a>).
...
# round(max(distance),2) == round(2 * maxlength,2)
...
lambda <- 0.7
corr_pagel <- lambda * covariance_matrix
corr_pagel <- corr_pagel - diag(diag(corr_pagel)) + diag(diag(covariance_matrix))


```

So, the way we simulate data assume a linear relationship between the relative distance $\Delta x$ between species and their expected covariance:
$$f(\Delta x) = \phi - \frac{\Delta x}{2}$$
When accounting for the Pagel's $\lambda$, we introduce a discontinuity:

```{r}
#| fig-height: 3.5
#| fig-width: 4

lambda <- 0.7 # as an example

cov_pagel <- lambda * covariance_matrix
cov_pagel <- corr_pagel - diag(diag(corr_pagel)) + diag(diag(covariance_matrix))

dataplot <- unique(data.frame(
  dist = round(c(distance),2),
  cov = round(c(cov_pagel),2)
))
...
grid()
axis(1, las = 1, cex.axis = 1, tck=-0.02, at = c(0,max(distance)), labels=c("0", expression(paste("2", varphi))))
axis(2, las = 1, cex.axis = 1, tck=-0.02, at = c(0,lambda*maxlength,maxlength), labels=c("0", expression(paste(lambda, "*", varphi)), expression(paste(varphi))))
title(main = "Covariogram", xlab = expression(paste(Delta,x)),  line=1, cex.main = 1)
