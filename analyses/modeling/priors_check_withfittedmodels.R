...
if(length(grep("victor", getwd())>0)) {
  setwd("/home/victor/projects/egret/analyses")
}
...
source("modeling/functions/toolbox.R")
...
# fit models
if(run_models){
  sm <- stan_model("stan/betaerrormodel.stan")
  ...
  plan(sequential);gc()
  saveRDS(datafit, file = "modeling/output/models_simulateddata_frompriors.rds")
}else{
  datafit <- readRDS(file = "modeling/output/models_simulateddata_frompriors.rds")
}

# plot simulated data
pdf(file=paste0("modeling/figures/priors_check.pdf"), height = 15, width = 18)
...
pdf(file=paste0("modeling/figures/priors_check_wfitintercept.pdf"), height = 15, width = 18)
par(mfrow = c(11,11), mar=c(1,0,0,0)+1)
for(i in 1:nplots){
...
# plot estimated intercepts
pdf(file=paste0("modeling/figures/priors_check_wfitslopes.pdf"), height = 15, width = 18)
par(mfrow = c(11,11), mar=c(1,0,0,0)+1)
for(i in 1:nplots){
