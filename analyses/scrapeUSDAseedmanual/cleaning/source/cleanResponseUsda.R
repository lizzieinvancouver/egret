...
d <- d %>%
  ...
  pivot_longer(cols = c("germination_time_days","total_germination_percent","avg_germination_percent","germination_rate",
                        "avg_germinative_energy_percent","germinative_energy_percent","avg_germinative_capacity",
                        "germinative_capacity","percent_germination_15degC_incubated","percent_germination_20degC_incubated",
                        "germ_rate_days","per50_germ"),
...
d$responseType[which(d$responseType == "germ_rate_days")] <- "germ.rate"
d$responseType[which(d$responseType == "per50_germ")] <- "50%germ"
...
# to match egret, using same names
d$responseVarClean[d$responseVarClean == "percent.germ.total"] <- "percent.germ"
d$responseVarClean[d$responseVarClean == "germ.capacity"] <- "percent.germ"
d$responseVarClean[d$responseVarClean == "mean.germ.capacity"] <- "percent.germ"
d$responseVarClean[d$responseVarClean == "percent.germ.15degC.incubated"] <- "percent.germ"
d$responseVarClean[d$responseVarClean == "percent.germ.20degC.incubated"] <- "percent.germ"

#another two that are probably the same
d$responseVarClean[d$responseVarClean == "mean.percent.germ.energy"] <- "germ.energy"
d$responseVarClean[d$responseVarClean == "percent.germ.energy"] <- "germ.energy"

d$responseVarClean[d$responseVarClean == "50%germ"] <- "50%.germ"

#replace the original response variable column with the cleaned one
# d$responseType <- d$responseVarClean
d$responseValue <- gsub("-"," to ", d$responseValue)
d$responseValue <- gsub("na",NA, d$responseValue)
d$responseValue[d$responseValue == "I"] <- "1"
