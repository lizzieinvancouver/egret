...
d<-read.csv("analyses/input/egretData.csv")
unique(d$genus)

goo<-d %>% group_by(respvar) %>% count()
d<-dplyr::filter(d)
...
b<-drm(response~germ.duration, data=dr,fct = LL.3(fixed = c(NA, NA, NA), names = c("b", "gmax", "e50")), type ="continuous")
summary(b)
plot(b)
