...
% <><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
\begin{enumerate}
\item First, we selected all the datasetIDstudy that had more than one provenance per species across the whole egret database (nrow $n = $ \Sexpr{nrow(dmore1prov)}. Second, we ran Victor's decision rules which yields \Sexpr{nrow(newd)} rows of data. Third, we cleaned the dataset before fitting the model with forcing, yielding \Sexpr{nrow(modeld)} rows of data. %emw11Aug: Did you select the studies BEFORE or after Victor's decision rules? If it is after, the I would write: After applying Victor's decision rules and cleaning the dataset, we selected all the .... (right now, it's not clear). Since you don't ref your R files above, I would note them in the comments here. 
...
\section{Results}
% <><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
For the dataset that includes forcing (henceforth referred to as `restricted'), we have \Sexpr{length(unique(modeld$genusspecies))} single species from \Sexpr{length(unique(paste(modeld$datasetID, modeld$study, sep = "")))}. For the dataset without forcing (full),
\begin{enumerate}
