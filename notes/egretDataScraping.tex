...
\item Excluding papers not in english unless the person scraping the data could read the language fluently %CRD26Setp2024 I think we decided to drop all papers that are not in english in the end, right?
...
\item If available we kept a minimum of two decimals per lat/long, but in some cases, vague coordinates of a region were provided with no decimals and we kept those indicating a less specific site location. % CRD24Sept2024 not sure if necessary here, but thought I mentioned it. If we want to keep, I could check if that is the case that we actually mentioned something in the column about non specific location. 
\end{itemize}
